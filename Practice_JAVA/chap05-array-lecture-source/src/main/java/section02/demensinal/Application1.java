...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        int[][] iArr1;
        int[] iArr2[];   // 닌자의 향기, 추천 X
        int iArr3[][];

        iArr1 = new int[3][2];     // 정변 배열을 위한 선언 방식 : 정사각형의 모양(관리하는 1차원 배열의 길이가 동일)
        iArr2 = new int[3][];      // 가변 배열을 위한 선언 방식 : 1차원의 배열의 길이가 다름(관리하는 1차원 배열의 길이가 다를 경우)
//        iArr3 = new int[][];     // 컴파일 에러 발생, 사용 X
        ...
        for (int i = 0; i < iArr2.length; i++) {         // 1차원 배열을 고르는 for문
            iArr2[i] = new int[++length];                // 자바는 반드시 크기를 할당해야함 (중요)
