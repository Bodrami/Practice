...
//    new *
    public static void main(String[] args) {          // main 함수이다.
        int num;                          /* 필기. 변수선언, num은 값을 담기 위한 공간(상수)*/
        num = 1;                          /* 필기. '='은 자바에서 좌항(num)을 공간으로 보고 리터럴 값을 넣는 대입 연산자이다. */
        ...
        /* 필기. 변수의 사용 목적
        *    1. 값의 의미를 부여하기 위한 목적 (의미 전달이 쉬어야 코드를 읽기 쉬워지고, 협업 및 유지보수에 유리하기 때문이다.)
        ...
        * */
        ...
        int salary = 1000000;                         // 변수명을 정해놓으면 유지보수, 프로젝트, 인수인계가 가능하다.
        int bonus = 200000;
        ...
        sum = sum + 10;                               // 코드는 위에서 아래로 실행된다.(더해진 값이 차례대로 치환됨)
        sum = sum + 10;
        sum = sum + 10;
        sum = sum + 10;
        System.out.println("sum에 10을 4번 누적하면 현재 sum의 값은: " + sum);
