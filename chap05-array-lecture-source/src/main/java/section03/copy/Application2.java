...
public class Application2 {
    ...
    public static void main(String[] args) {
        ...
        int[] ogiginArr = new int[]{1, 2, 3, 4};
        print(ogiginArr, "원본");

        /* 목차 1. for문 활용 */
        int[] copyArr1 = new int[ogiginArr.length];
        for (int i = 0; i < ogiginArr.length; i++) {
            copyArr1[i] = ogiginArr[i];
        }
        print(copyArr1, "for문을 사용한 사본");

        /* 목차 2. clone()를 이용한 복사 */
        int[] copyArr2 = ogiginArr.clone();
        print(copyArr2, "clone()을 사용한 사본");

        /* 목차 3. arraycopy()를 이용한 복사 */
        int[] copyArr3 = new int[ogiginArr.length + 3];
        System.arraycopy(ogiginArr, 0, copyArr3, 3, ogiginArr.length);
        print(copyArr3, "arraycopy()를 사용한 사본");

        /* 목차 4. copyOf()를 이용한 복사 */
        int[] copyArr4 = Arrays.copyOf(ogiginArr, 2);  // 원본의 처음부터 원하는 길이까지만 복사 가능
