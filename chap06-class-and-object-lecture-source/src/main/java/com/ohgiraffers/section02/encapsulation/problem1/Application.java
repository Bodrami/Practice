...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        /* 설명. 2번 몬스터 생성 (문제점 확인: -200체력에서 '-'값이 들어가지면 안되는데 들어가짐.)*/
        Monster monster2 = new Monster();
        monster2.name = "뿌꾸";
        monster2.hp = -200;       // 여기서는 조건문을 넣을 수 없음. setHP

        System.out.println("monster2의 이름: " + monster2.name);
        System.out.println("monster2의 체력: " + monster2.hp);

        /* 설명. 3번 몬스터 생성 (setter를 통한 접근) */
        Monster monster3 = new Monster();  // new로 Monster 객체를 만들었다. 객체 안 객체명: name, hp
        monster3.name = "드라쿨라";
        monster3.setHp(-100);     // 메소드 호출 시 non-static 으로 생성되고, monster3의 hp 공간에 100이 할당

        System.out.println("monster3의 이름: " + monster3.name);
        System.out.println("monster3의 체력: " + monster3.hp);

        /* 설명. 4번 몬스터 생성(satter를 통한 접근) */
